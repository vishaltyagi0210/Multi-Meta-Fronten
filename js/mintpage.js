...
mint_btn.addEventListener("click", async () => {
  if (window.ethereum) {
    ...
    const MultiMetaContract = new ethers.Contract(
      MultiMetaAddress,
      MultiMeta.abi,
      signer
    );
    ...
    try {
      const response = await MultiMetaContract.mint(mintAmount, {
        value: ethers.utils.parseEther((0.12 * mintAmount).toString()),
      });
    } catch (error) {
        alert("error", formatError(error));
        console.log('there', formatError(error));
  ...
  (error = {
    code: -32000,
    message:
      "err: insufficient funds for gas * price + value: address 0xCD4B748ccA60fC0572669b443116Aa82ED2EB5b1 have 109432739395631202 want 120000000000000000 (supplied gas 15010499)",
  })
);
